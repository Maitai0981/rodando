...
export const api = {
  ...
  deleteAddress: (id: number) =>
    fetch(`${API_BASE}/api/auth/addresses/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    }).then(async (res) => {
      if (!res.ok) {
  ...
  deleteShippingPromotion: (id: number) =>
    fetch(`${API_BASE}/api/owner/shipping-promotions/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    }).then(async (res) => {
      if (!res.ok) {
  ...
  deleteOwnerOffer: (id: number) =>
    fetch(`${API_BASE}/api/owner/offers/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    }).then(async (res) => {
      if (!res.ok) {
  ...
  deleteProduct: (id: number, mode: 'hard' | 'archive' = 'hard') =>
    fetch(`${API_BASE}/api/owner/products/${id}?mode=${encodeURIComponent(mode)}`, {
      method: 'DELETE',
      credentials: 'include',
    }).then(async (res) => {
      ...
      if (!res.ok) {
        const message = payload?.error || payload?.message || `Erro HTTP ${res.status}`
        throw new ApiError(message, res.status, payload?.code, payload)
      }
      return payload as { item?: Product; archived?: boolean } | null
    }),
}
