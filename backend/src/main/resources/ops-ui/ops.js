...
function pulseElement(element) {
  if (!element) return
  element.classList.remove('ops-updated')
  window.requestAnimationFrame(() => {
    element.classList.add('ops-updated')
    window.setTimeout(() => {
      element.classList.remove('ops-updated')
    }, 800)
  })
...
function setActiveTab(nextTabId, { persist = true, focusButton = false } = {}) {
  ...
  state.activeTab = selectedButton.dataset.tab || 'status'
...
async function loadStatus() {
  ...
  try {
    ...
    ;[
      statusElements.summaryHealth?.closest('.summary-card'),
      statusElements.summaryReady?.closest('.summary-card'),
      statusElements.summaryRequestsTotal?.closest('.summary-card'),
      statusElements.summaryCacheHit?.closest('.summary-card'),
      statusElements.health?.closest('.status-card'),
      statusElements.ready?.closest('.status-card'),
      statusElements.metrics?.closest('.status-card'),
    ].forEach((node) => pulseElement(node))
  } catch (error) {
...
async function loadRequests() {
  ...
  try {
    ...
    pulseElement(requests.tbody?.closest('.table-wrap'))
  } catch (error) {
    const message = String(error?.message || error)
...
async function loadTables() {
  ...
  try {
    ...
    pulseElement(db.tableList?.closest('.db-tables'))
  } catch (error) {
    const message = String(error?.message || error)
...
async function loadTablePreview() {
  ...
  try {
    ...
    pulseElement(db.previewBody?.closest('.table-wrap'))
  } catch (error) {
    const message = String(error?.message || error)
...
function renderSqlResult(data) {
  ...
  pulseElement(sql.resultBody?.closest('.table-wrap'))
  pulseElement(sql.resultMeta)
}
